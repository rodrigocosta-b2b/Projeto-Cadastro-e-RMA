...
// baixado por todo mundo (inclusive quem só abre o portal do revendedor).
...
const MOTIVOS = [
  { name: "Fora do prazo", value: 38, color: C.danger },
...
function Shell({ portal, switchPortal, view, nav, onLogout, notifOpen, setNotifOpen, user, notifs, badge, onOpenNotifs, themePref, onCycleTheme, requests, children }) {
  ...
  // Busca global: interno vê todas as solicitações; revendedor só vê as suas.
    ...
    <div className="min-h-screen" style={{ background: C.bg }}>
      <header className="h-16 flex items-center px-4 gap-4 text-white sticky top-0 z-30" style={{ background: C.coral }}>
        ...
        <div onClick={() => nav(portal === "externo" ? "inicio" : "dash")} className="flex items-center gap-2 w-auto md:w-60 cursor-pointer shrink-0">
          ...
          <span className="text-xs opacity-80 hidden sm:inline">Service Desk</span>
        </div>
    ...
    <div>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-5">
          ...
          {r.tipo === "Troca/Garantia" && (
            <Card className="p-5">
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-bold" style={{ color: C.text }}>Dados da venda e produto</h2>
                <div className="text-xs" style={{ color: C.muted }}>Data da venda: <span className="font-medium" style={{ color: C.text }}>{r.venda}</span></div>
              </div>
              {r.itens && r.itens.length > 0 ? (
                <div className="space-y-3">
                  {r.itens.map((it, i) => (
                    <div key={i} className="rounded-xl border p-3" style={{ borderColor: C.line }}>
                      {r.itens.length > 1 && <div className="text-xs font-semibold mb-2" style={{ color: C.coral }}>Item {i + 1}</div>}
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        {[["Produto", it.produto], ["Modelo do aparelho", it.modelo], ["Nota fiscal", it.nf]].map(([k, v]) =>
                          <div key={k}><div className="text-xs" style={{ color: C.muted }}>{k}</div><div className="font-medium" style={{ color: C.text }}>{v}</div></div>)}
                      </div>
                      {it.problema && <div className="mt-2"><div className="text-xs" style={{ color: C.muted }}>Problema relatado</div><p className="text-sm" style={{ color: C.text }}>{it.problema}</p></div>}
                      {it.foto && it.foto.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-2">
                          {it.foto.map((a, fi) => (
                            <button key={fi} onClick={() => abrirDoc(a, null)} className="w-16 h-16 rounded-lg border overflow-hidden" style={{ borderColor: C.line }}>
          ...
          {r.nfd && r.nfd.length > 0 && (
            <Card className="p-5" style={{ borderColor: C.green + "50", background: C.green + "08" }}>
              <div className="flex items-center gap-2 mb-3">
                <FileCheck size={16} style={{ color: C.green }} />
                <h2 className="font-bold text-sm" style={{ color: C.text }}>Nota fiscal de devolução (NFD) enviada pelo cliente</h2>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {r.nfd.map((a, i) => {
                  const isImg = (a.type || "").startsWith("image"); const isPdf = (a.type || "").includes("pdf");
                  return (
                    <button key={i} onClick={() => abrirDoc(a, null)} className="block rounded-xl border overflow-hidden text-left w-full hover:shadow-md transition bg-white" style={{ borderColor: C.line }}>
                      <div className="h-24 flex items-center justify-center bg-gray-50">
                        {isImg ? <img src={a.url} alt={a.name} className="h-full w-full object-cover" /> : <div style={{ color: C.coral }}>{isPdf ? <FileIcon size={26} /> : <Paperclip size={26} />}</div>}
                      </div>
                      <div className="p-2">
                        <div className="text-xs font-medium truncate" style={{ color: C.text }}>{a.name}</div>
                        <div className="text-[10px] flex items-center gap-1 font-semibold" style={{ color: C.coral }}><ExternalLink size={10} />Abrir / baixar</div>
                      </div>
                    </button>
                  );
                })}
...
function ExtNovaTG({ nav, toast, onCreate, user }) {
  ...
  const novoItem = () => ({ _key: itemKeyRef.current++, produto: "", modelo: "", nf: "", problema: "", foto: [] });
  const [itens, setItens] = useState(() => [{
    _key: itemKeyRef.current++, produto: "Capa Anti-Impacto Pro", modelo: "iPhone 15 Pro", nf: "045231",
    problema: "Capa anti-impacto com trinca na quina após 1 mês de uso normal.", foto: [],
  }]);
  const setR_ = (k) => (e) => setR(s => ({ ...s, [k]: e.target.value }));
  const setItemField = (idx, k) => (e) => setItens(list => list.map((it, i) => i === idx ? { ...it, [k]: e.target.value } : it));
  const setItemFoto = (idx) => (files) => setItens(list => list.map((it, i) => i === idx ? { ...it, foto: files } : it));
  const addItem = () => {
    if (itens.length >= MAX_ITENS_TG) { toast(`Limite de ${MAX_ITENS_TG} itens por solicitação.`); return; }
  ...
  const enviar = () => {
    ...
    for (let i = 0; i < itens.length; i++) {
      const it = itens[i];
      if (!it.produto.trim() || !it.nf.trim()) { toast(`Item ${i + 1}: informe o produto e o número da nota fiscal de venda.`); return; }
      if (!it.foto || it.foto.length === 0) { toast(`Item ${i + 1}: anexe a foto do produto com avaria.`); return; }
    }
    if (!nfd || nfd.length === 0) { toast("Anexe a nota fiscal de devolução (NFD) para adiantarmos a verificação da troca."); return; }
    ...
    onCreate({
      ...
      email: r.email,
      produto: primeiro.produto + (itens.length > 1 ? ` (+${itens.length - 1} ${itens.length === 2 ? "item" : "itens"})` : ""),
      modelo: primeiro.modelo, nf: primeiro.nf,
      venda: venda.split("-").reverse().join("/"),
      problema: primeiro.problema,
      itens: itens.map(({ _key, ...it }) => it),
      anexos: itens.flatMap(it => it.foto || []),
      nfd,
    });
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="flex items-end justify-between mb-3 gap-3">
        <div>
          <h2 className="text-sm font-semibold" style={{ color: C.text }}>Produtos da solicitação</h2>
          <p className="text-xs" style={{ color: C.muted }}>Envie até {MAX_ITENS_TG} produtos em uma única solicitação. Cada item precisa da foto do produto com avaria e do número da nota fiscal de venda.</p>
        </div>
        <span className="shrink-0 text-xs font-semibold rounded-full px-2.5 py-1" style={{ color: C.coral, background: C.coralSoft }}>{itens.length}/{MAX_ITENS_TG}</span>
      </div>

      <div className="space-y-3 mb-3">
        {itens.map((it, idx) => (
          <Card key={it._key} className="p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm font-semibold" style={{ color: C.text }}>Item {idx + 1}</div>
              {itens.length > 1 && (
                <button onClick={() => removeItem(idx)} className="flex items-center gap-1 text-xs font-semibold" style={{ color: C.coral }}>
                  <X size={14} /> Remover item
                </button>
              )}
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              <Field label="Produto gocase" value={it.produto} onChange={setItemField(idx, "produto")} />
              <Field label="Modelo do aparelho" value={it.modelo} onChange={setItemField(idx, "modelo")} />
              <Field label="Número da nota fiscal de venda" value={it.nf} onChange={setItemField(idx, "nf")} />
            </div>
            <label className="text-xs font-semibold mt-4 block" style={{ color: C.muted }}>Descrição do problema</label>
            <textarea rows={3} value={it.problema} onChange={setItemField(idx, "problema")} className="mt-1 w-full rounded-xl border px-3 py-2 text-sm bg-white" style={{ borderColor: C.line }} />
            <label className="text-xs font-semibold mt-4 block" style={{ color: C.muted }}>Foto do produto com avaria</label>
            <div className="mt-1"><FileUpload accept="image/jpeg,image/png,image/webp" hint="Clique ou arraste para adicionar a foto (JPG, PNG, WEBP)" initial={it.foto} onFiles={setItemFoto(idx)} /></div>
          </Card>
        ))}
      </div>

      <button onClick={addItem} disabled={itens.length >= MAX_ITENS_TG}
        className="w-full mb-4 rounded-xl border border-dashed py-2.5 text-sm font-semibold flex items-center justify-center gap-1.5 disabled:opacity-40"
        style={{ borderColor: C.line, color: C.coral }}>
        <Plus size={15} /> Adicionar outro produto
      </button>

      <Card className="p-5 mb-4">
        <label className="text-xs font-semibold" style={{ color: C.muted }}>Data de venda ao cliente final</label>
        <p className="text-xs mt-0.5 mb-1" style={{ color: C.muted }}>Usada pela IA para validar o prazo de garantia da solicitação.</p>
        <input type="date" value={venda} onChange={e => setVenda(e.target.value)} className="mt-1 w-full sm:w-64 rounded-xl border px-3 py-2 text-sm bg-white" style={{ borderColor: C.line }} />
      </Card>

      <Card className="p-5 mb-4">
        <div className="flex items-center gap-2 text-sm font-semibold mb-1" style={{ color: C.coral }}><FileCheck size={16} /> Nota fiscal de devolução (NFD)</div>
        <p className="text-xs mb-3" style={{ color: C.muted }}>Encaminhe já a NFD para que possamos adiantar a verificação da troca.</p>
        <FileUpload accept="image/jpeg,image/png,image/webp,application/pdf" hint="Clique ou arraste para adicionar a NFD (JPG, PNG, WEBP ou PDF)" initial={nfd} onFiles={setNfd} />
      </Card>
...
function Manual() {
  ...
  const docs = [
    ...
    { t: "Guia de Emissão de NFD", d: "Passo a passo para emitir a Nota Fiscal de Devolução nas trocas e garantias.", icon: FileCheck, action: "pdf" },
  ];
  return (
    <div>
    ...
    <div>
      ...
      {list.length > 0 && (
        <Card className="p-3 mb-4">
          <div className="relative">
            ...
            <input value={q} onChange={e => setQ(e.target.value)} placeholder="Buscar por revendedor, CNPJ, UF ou contato…"
      ...
      <Card>
        {filtered.length === 0 ? (
          ...
          <DataTable rows={paged} cols={["Revendedor", "CNPJ", "UF", "Contato", "Solicitações", ""]} onRow={c => nav("cliente", c.nome)} render={c => (
            <>
  ...
  return (
    <div className="max-w-2xl">
      ...
      <Card className="p-5 mb-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4 text-sm">
          <div><label className="text-xs font-semibold" style={{ color: C.muted }}>Prazo de garantia (meses)</label><input defaultValue="6" className="mt-1 w-full rounded-xl border px-3 py-2 bg-white" style={{ borderColor: C.line }} /></div>
...
function Historico({ nav }) {
  ...
  const { paged, more } = usePagedList(rows, `${q}|${fStatus}|${fAno}`);
  ...
  return (
    <div>
      ...
      <div className="grid sm:grid-cols-5 gap-3 mb-4">
        ...
        <StatCard icon={XCircle} label="Negadas" value={count("NEGADA")} color={C.danger} />
      ...
      <Card>
        {rows.length === 0 ? (
          ...
          <DataTable rows={paged} cols={["Número", "Revendedor", "UF", "Produto", "Status", "Abertura", ""]} onRow={r => nav("hist-detalhe", r.id)} render={r => (
            <>
              <td className="px-4 py-3 font-mono font-semibold" style={{ color: C.coral }}>{r.id}</td>
              <td className="px-4 py-3" style={{ color: C.text }}>{r.parceiro}</td>
              <td className="px-4 py-3" style={{ color: C.muted }}>{r.uf}</td>
        ...
        <LoadMore shown={paged.length} total={rows.length} onMore={more} />
      </Card>
      {rows.length > 0 && (
        <p className="text-xs mt-3" style={{ color: C.muted }}>
          Mostrando {paged.length} de {rows.length}{rows.length !== HISTORICO.length ? ` (filtrados de ${HISTORICO.length})` : ""} registros.
        </p>
      )}
